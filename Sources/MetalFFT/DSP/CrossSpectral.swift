...
public extension PSD {
    ...
    static func crossSpectral(
        ...
        while pos + N <= signalLen {
            ...
            for c in 0 ..< C {
                signals[c].withUnsafeBufferPointer { ptr in
                    ...
                    for i in 0 ..< N {
                        windowedChannels[c][i] = SIMD2<Float>(windowed[i], 0)
                    }
                }
            }
            ...
            for p in 0 ..< pairs {
                ...
                for k in 0 ..< N {
                    avgCross[p][k].x += cs[k].x; avgCross[p][k].y += cs[k].y
                }
        ...
        for p in 0 ..< pairs {
            for k in 0 ..< N {
                avgCross[p][k].x *= invFrames; avgCross[p][k].y *= invFrames
            }
