...
public enum FFTError: Error, CustomStringConvertible {
    ...
    public var description: String {
        ...
        case .noCommandQueue: "Could not create Metal command queue"
        case let .libraryBuildFailed(e): "Metal library build failed: \(e)"
        case let .kernelNotFound(name): "Metal kernel not found: \(name)"
        case let .commandBufferFailed(msg): "Command buffer error: \(msg)"
        case let .invalidInputSize(e, g): "Input size mismatch: expected \(e), got \(g)"
        case let .batchInputSize(e, g, i): "Batch element \(i): expected \(e), got \(g)"
        case let .unsupportedFFTSize(n): "Unsupported FFT size: \(n). Supported: 64,128,256,512,1024,2048,4096,8192,16384"
        case .bufferAllocationFailed: "Metal buffer allocation failed"
        }
    }
}
